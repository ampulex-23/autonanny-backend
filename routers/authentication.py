...
async def registration_parent(item: RegistrationParent):
    from config import settings
    from common.logger import log_business_event
    ...
    # Автоматическое закрепление за франшизой по умолчанию (MVP)
    await UsersFranchiseUser.create(
        id_user=user.id,
        id_franchise=settings.default_franchise_id
    )
    ...
    log_business_event(
        ...
        franchise_name=settings.default_franchise_name
    )
...
async def registration_driver(item: RegistrationDriver):
    ...
    # Автоматическое закрепление за франшизой (MVP)
    from config import settings
    from common.logger import log_business_event
    ...
    try:
        if item.refCode is None:
            # Нет реферального кода - закрепляем за франшизой по умолчанию
            franchise_id = settings.default_franchise_id
        else:
            # Есть реферальный код - пытаемся найти партнера
            partners = [
                x["id_user"]
                for x in (
            ...
            if (
                ...
                franchise_id = franchise["id_franchise"]
            else:
                franchise_id = settings.default_franchise_id
    except Exception:
        franchise_id = settings.default_franchise_id
    
    # Создаём привязку к франшизе
    await UsersFranchiseUser.create(id_user=user.id, id_franchise=franchise_id)
    ...
    log_business_event(
        ...
        has_refcode=item.refCode is not None
    )
